use nom::{branch::alt, bytes::streaming::{tag, take}, combinator::{eof, map, value}, number::streaming::{le_u32, le_u64, le_u8}, Err, Parser};
...
impl<'a> OggPage<'a> {
    pub fn parse_headers(input: &'a[u8]) -> Result<(&'a [u8], OggPage<'a>), ()> {
        ...
        let (remaining, stream_structure_version) = le_u8(remaining).map_err(|_: Err<(), ()>| ())?;
        let (remaining, header_type_flag) = le_u8(remaining).map_err(|_: Err<(), ()>| ())?;
        let (remaining, absolute_granule_position) = le_u64(remaining).map_err(|_: Err<(), ()>| ())?;
        ...
        let (remaining, segments_table) = take(page_segments)(remaining).map_err(|_: Err<(), ()>| ())?;

        Ok((remaining, OggPage {
                capture_pattern,
    ...
    pub fn parse_segments(input: &'a[u8], segments_table: &'a[u8]) -> Result<(&'a [u8], &'a [u8]), ()> {
